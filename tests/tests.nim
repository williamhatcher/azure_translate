...
import unittest, options, os, tables, strutils, re, httpclient
...
test "invalid key":
    expect AzureTranslateError:
        discard translate(client, "", @[""], @[""])

suite "languages":
    let all = languages(client, key)
    ...
    test "single language":
        ...
        let single = languages(client, key, @[azDictionary])
    ...
    test "multiple languages":
        ...
        let multiple = languages(client, key, @[azTransliteration, azTranslation])
...
suite "translate":
    ...
    test "single input":
        let translation = translate(client, key, en_text, to = @["zh-Hans"], `from` = some("en"))[0].translations[0]

        check(translation.to == "zh-Hans")
        check(not translation.text.isEmptyOrWhitespace)

    test "single input auto-detection":
        let translation = translate(client, key, en_text, @["es"])[0]
    ...
    test "transliteration":
        let translation = translate(client, key, en_text, @["zh-Hans"], toScript=some("Latn"))[0]
    ...
    test "multiple inputs":
        let translations = translate(client, key, text = en_text & @["I am fine, thank you."], to = @["zh-Hans"], `from` = some("en"))
    ...
    test "multiple languages":
        let translations = translate(client, key, en_text, to = @["zh-Hans", "de"], `from` = some("en"))[0].translations
    ...
    test "profanity":
        let asterisk = translate(client, key, @["This is a fucking good idea."], to = @["de"], `from` = some("en"), profanityAction=some(ProfanityAction.marked))[0]
            .translations[0]
        
        check(asterisk.to == "de")
        check("***" in asterisk.text)

        let tag = translate(client, key, @["This is a fucking good idea."], to = @["de"], `from` = some("en"), profanityAction=some(ProfanityAction.marked), profanityMarker=some(ProfanityMarker.tag))[0]
            .translations[0]
    ...
    test "notranslate":
        ...
        let translation = translate(client, key, content_markup_text, to = @["zh-Hans"], `from` = some("en"), textType = some(TextType.html))[0]
            .translations[0]
    ...
    test "alignment":
        let translation = translate(client, key, @["The answer lies in machine translation."], to = @["fr"], some("en"), includeAlignment=some(true))[0]
            .translations[0]
    ...
    test "sentence boundaries":
        ...
        let translation = translate(client, key, sentence_boundary_text, @["fr"], some("en"), includeSentenceLength=some(true))[0]
            .translations[0]
    ...
    test "dynamic dictionary":
        ...
        let translation = translate(client, key, dynamic_dictionary_text, @["de"], some("en"))[0]
            .translations[0]
        check(translation.to == "de")
        check("wordomatic" in translation.text)

test "transliterate":
    let res = transliterate(client, key, @["こんにちは", "さようなら"], "ja", "Jpan", "Latn")
...
test "detect":
    let res = detect(client, key, @["What language is this text written in?", "이 텍스트는 어떤 언어로 쓰여져 있습니까?"])
    check(res[0].language == "en")
    check(res[1].language == "ko")

test "break setence":
    let res = break_sentence(client, key, @["How are you? I am fine. What did you do today?"], language=some("en"))
    check(res[0].sentLen == @[Natural(13), 11, 22])  # Compiler assumes int if Natural is missing

test "dictionary lookup":
    let entry = dictionary_lookup(client, key, @["fly"], `from`="en", to="es")[0]  # Getting first! (may need to change in the future)
    check(entry.normalizedSource == "fly")
    check(entry.displaySource == "fly")
    check(entry.translations.len > 0)

test "dictionary examples":
    let example = dictionary_examples(client, key, @[(text: "fly", translation: "volar")], `from`="en", to="es")[0]
